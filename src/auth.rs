    ...
    fn authorize(&self, _pr: (), req: &Request) -> Result<()> {
        ...
        if let Some(cr) = req.header::<Authorization<Bearer>>() {
            if self.cred.as_ref().unwrap() == &*cr.0.token {
               ...
               Err(gen_unauthorized("Unauthorized access. Please check if your \
                    identity token is correct."))
            }
        } else {
            Err(gen_unauthorized("This request needs certain priviledge to be \
                proceeded. Please provide your identity token in header \
                `Authorization`."))
        }
    }
}
